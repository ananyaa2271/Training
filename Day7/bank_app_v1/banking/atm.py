    ...
    def _do_close(self):
        if self._keyboard('To Close Account type "CLOSE ACCOUNT"?')=="CLOSE ACCOUNT":
            ...
            if amount!=None:
                ...
                self._show_message('account closure failed')


    def _do_deposit(self):
        amount = self._keyboard('Amount to Deposit? ',int)
        if self._bank.deposit(self._account_number, amount):
            self._show_message('Amount Deposited')
        else:
            self._show_message('Amount Deposit Failed')

    def _do_withdraw(self):
        amount = self._keyboard('Amount to Withdraw? ',int)
        
        if self._bank.withdraw(self._account_number, amount,self._password):
            self._dispense_cash(amount)
        else:
            self._show_message('Amount withdraw Failed')


    def _do_transfer(self):
        amount = self._keyboard('Amount to Transfer? ',int)
        to_account=self._keyboard('Transfer to ?', int)
        if self._bank.transfer(self._account_number, amount,self._password,to_account):
